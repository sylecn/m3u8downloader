...
def test_rewrite_key_uri():
    assert (
        rewrite_key_uri(
            "/tmp/foo",
            "https://example.com/foobar/foo.m3u8",
            '#EXT-X-KEY:METHOD=AES-128,URI="foo.key"',
        )
        == '#EXT-X-KEY:METHOD=AES-128,URI="/tmp/foo/foobar/foo.key"'
    )
    assert (
        rewrite_key_uri(
            "/tmp/foo",
        ...
        == '#EXT-X-KEY:METHOD=AES-128,URI="/tmp/foo/foo.key"'
    )
    assert (
        rewrite_key_uri(
            "/tmp/foo",
            "https://example.com/foobar/foo.m3u8",
            '#EXT-X-KEY:METHOD=AES-128,URI="/abc/foo.key"',
        )
        == '#EXT-X-KEY:METHOD=AES-128,URI="/tmp/foo/abc/foo.key"'
    )
    assert (
        rewrite_key_uri(
            "/tmp/foo",
            "https://example.com/foobar/foo.m3u8",
            '#EXT-X-KEY:METHOD=AES-128,URI="https://example.com/abc/foo.key"',
        )
        == '#EXT-X-KEY:METHOD=AES-128,URI="/tmp/foo/abc/foo.key"'
    )
    assert (
        ...
        == '#EXT-X-KEY:METHOD=AES-128,URI="C:/Users/foo/temp/abc/foo.key"'
    )
...
def test_get_local_file_for_url():
    ...
    assert (
        get_local_file_for_url("/tmp/foo", "http://example.com/abc/def.ts")
        == "/tmp/foo/abc/def.ts"
    )
    assert (
        get_local_file_for_url("/tmp/foo", "https://example.com/abc/def.ts")
        == "/tmp/foo/abc/def.ts"
    )
    assert (
        get_local_file_for_url(
            "/tmp/foo", "https://example.com/abc/def.ts", "/tmp/foo/abc/def.ts"
        )
        == "/tmp/foo/abc/def.ts"
    )
...
def test_join():
    assert (
        os.path.normpath(os.path.join("/foo/bar/baz", "./abc.txt"))
        == "/foo/bar/baz/abc.txt"
    )
...
def test_get_url_path():
    assert (
        get_url_path("http://example.com/250kb/hls/index.m3u8")
        == "/250kb/hls/index.m3u8"
    )
...
def test_get_fullpath():
    ...
    assert get_fullpath("foo/") == os.path.abspath(os.path.join(os.curdir, "foo"))
    assert get_fullpath("foo/bar") == os.path.abspath(
        os.path.join(os.curdir, "foo", "bar")
    )
